...
_prep_sslutils_windows() {
    ...
    ssh $SSLU_SSH "cd $SSLU_REPO; cmd /c del /F /Q vc-build-sslutils.bat"
    ...
    # Cleanup local files
    if [ -f $SSLU_PACKAGE_PATH/scripts/$SSLU_PLATFORM/vc-build-sslutils.bat ];
    then
        echo "Removing existing vc-build-sslutils script"
        rm -rf $SSLU_PACKAGE_PATH/scripts/$SSLU_PLATFORM/vc-build-sslutils.bat || _die "Couldn't remove the existing vc-build-sslutils script"
    fi
    mkdir -p $SSLU_PACKAGE_PATH/scripts/$SSLU_PLATFORM

    # Copy sources on windows VM
    echo "Copying pem_agent sources to Windows VM"
    rsync -av sslutils-$SSLU_PLATFORM.zip $SSLU_SSH:$PG_CYGWIN_PATH_WINDOWS || _die "Couldn't copy the sslutils archieve to windows VM (sslutils-$SSLU_PLATFORM.zip)"
    ssh $SSLU_SSH "cd $SSLU_REPO; cmd /c unzip sslutils-$SSLU_PLATFORM.zip" || _die "Couldn't extract sslutils archieve on windows VM (sslutils-$SSLU_PLATFORM.zip)"
...
EOT
    ...
    scp vc-build-sslutils.bat  $SSLU_SSH:$SSLU_SOURCE_PLAT || _die "Failed to copy the vc-build-sslutils.bat  to the windows build host"
    ssh $SSLU_SSH "cd $SSLU_SOURCE_PLAT; cmd /c vc-build-sslutils.bat" || _die "Failed to build sslutils on the build host"

    mkdir -p $SSLU_STAGING/lib
    mkdir -p $SSLU_STAGING/share
    mkdir -p $SSLU_STAGING/doc

    echo "Copying libraries from the windows VM to staging directory"
...
_postprocess_sslutils_windows() {
    ...
    echo "#####################################"
    echo "# sslutils : WIN : Post Process #"
    echo "#####################################"
